...
// Import Zod schemas
import { createUserInputSchema, createProductInputSchema, updateProductInputSchema, createProductImageInputSchema, updateProductImageInputSchema, createCategoryInputSchema, updateCategoryInputSchema, createAddressInputSchema, updateAddressInputSchema, createCartItemInputSchema, updateCartItemInputSchema, createOrderInputSchema, createWishlistItemInputSchema, createProductReviewInputSchema, createContactMessageInputSchema, createNewsletterSubscriptionInputSchema, createInventoryAdjustmentInputSchema } from './schema.js';
dotenv.config();
...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        try {
            const result = await client.query('SELECT user_id, email, first_name, last_name, phone, email_verified, is_active, created_at, updated_at FROM users WHERE user_id = $1 AND is_active = true', [decoded.user_id]);
...
const authenticateAdmin = async (req, res, next) => {
    ...
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query('SELECT admin_id, username, email, role, permissions, is_active, created_at, updated_at FROM admin_users WHERE admin_id = $1 AND is_active = true', [decoded.admin_id]);
            if (result.rows.length === 0) {
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        try {
            ...
            const userResult = await client.query(`INSERT INTO users (user_id, email, password_hash, first_name, last_name, phone, email_verified, is_active, created_at, updated_at) 
         VALUES ($1, $2, $3, $4, $5, $6, false, true, $7, $8) 
         RETURNING user_id, email, first_name, last_name, phone, email_verified, is_active, created_at, updated_at`, [userId, email.toLowerCase(), password_hash, first_name, last_name, phone, now, now]);
            ...
            await client.query(`INSERT INTO user_sessions (session_id, user_id, token_hash, expires_at, ip_address, user_agent, is_active, created_at) 
         VALUES ($1, $2, $3, $4, $5, $6, true, $7)`, [sessionId, userId, token, expiresAt, req.ip, req.get('User-Agent'), now]);
            res.status(201).json({
                user,
                token,
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        try {
            ...
            const result = await client.query('SELECT user_id, email, password_hash, first_name, last_name, phone, email_verified, is_active, created_at, updated_at FROM users WHERE email = $1 AND is_active = true', [email.toLowerCase()]);
            if (result.rows.length === 0) {
            ...
            const token = jwt.sign({ user_id: user.user_id, email: user.email }, JWT_SECRET, { expiresIn });
            // Create session record
            const sessionId = uuidv4();
            const expiresAt = new Date(Date.now() + (remember_me ? 30 : 7) * 24 * 60 * 60 * 1000).toISOString();
            const now = new Date().toISOString();
            await client.query(`INSERT INTO user_sessions (session_id, user_id, token_hash, expires_at, ip_address, user_agent, is_active, created_at) 
         VALUES ($1, $2, $3, $4, $5, $6, true, $7)`, [sessionId, user.user_id, token, expiresAt, req.ip, req.get('User-Agent'), now]);
...
app.post('/api/auth/logout', authenticateToken, async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            await client.query('UPDATE user_sessions SET is_active = false WHERE token_hash = $1', [token]);
...
app.get('/api/products', async (req, res) => {
    try {
        const { search_query, category_id, brand, price_min, price_max, is_featured, in_stock, sort_by = 'created_at', sort_order = 'desc', limit = 20, offset = 0 } = req.query;
...
app.patch('/api/products/:product_id', async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query('UPDATE products SET view_count = view_count + 1 WHERE product_id = $1 AND is_active = true RETURNING view_count', [product_id]);
            if (result.rows.length === 0) {
                return res.status(404).json(createErrorResponse('Product not found', null, 'PRODUCT_NOT_FOUND'));
            }
...
app.post('/api/cart', optionalAuth, async (req, res) => {
    try {
        ...
        try {
            ...
            const productResult = await client.query('SELECT product_id, stock_quantity FROM products WHERE product_id = $1 AND is_active = true', [product_id]);
            if (productResult.rows.length === 0) {
            ...
            if (existingItemResult.rows.length > 0) {
                ...
                await client.query('UPDATE cart_items SET quantity = $1, updated_at = $2 WHERE cart_item_id = $3', [newQuantity, now, existingItem.cart_item_id]);
            }
            else {
                // Create new cart item
                const cartItemId = uuidv4();
                await client.query(`INSERT INTO cart_items (cart_item_id, user_id, session_id, product_id, quantity, added_at, updated_at) 
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [cartItemId, userId, session_id, product_id, quantity, now, now]);
            }
            // Return updated cart
...
app.patch('/api/cart/:cart_item_id', optionalAuth, async (req, res) => {
    try {
      ...
      `;
            ...
            // Update quantity
            const now = new Date().toISOString();
            await client.query('UPDATE cart_items SET quantity = $1, updated_at = $2 WHERE cart_item_id = $3', [quantity, now, cart_item_id]);
            // Return updated cart
    ...
    try {
        ...
        try {
            ...
            for (const item of cartItems) {
                const productResult = await client.query('SELECT stock_quantity, price, sale_price FROM products WHERE product_id = $1 AND is_active = true', [item.product_id]);
                if (productResult.rows.length === 0) {
      ...
      `, [
        ...
        `, [
                ...
                const inventoryResult = await client.query('UPDATE products SET stock_quantity = stock_quantity - $1, sales_count = sales_count + $2, updated_at = $3 WHERE product_id = $4 RETURNING stock_quantity', [item.quantity, item.quantity, now, item.product_id]);
...
app.get('/api/orders/:order_id', optionalAuth, async (req, res) => {
    try {
      ...
      `;
            ...
            const shippingAddressResult = await client.query('SELECT * FROM addresses WHERE address_id = $1', [order.shipping_address_id]);
            const billingAddressResult = await client.query('SELECT * FROM addresses WHERE address_id = $1', [order.billing_address_id]);
    ...
    try {
        ...
        try {
            ...
            if (email !== undefined) {
                ...
                const emailCheckResult = await client.query('SELECT user_id FROM users WHERE email = $1 AND user_id != $2', [email.toLowerCase(), userId]);
...
app.post('/api/user/change-password', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            ...
            await client.query('UPDATE users SET password_hash = $1, updated_at = $2 WHERE user_id = $3', [new_password, new Date().toISOString(), userId]);
    ...
    try {
        ...
        try {
            ...
            if (validatedData.is_default) {
                await client.query('UPDATE addresses SET is_default = false WHERE user_id = $1 AND type = $2', [validatedData.user_id, validatedData.type]);
            }
...
app.get('/api/user/addresses/:address_id', authenticateToken, async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query('SELECT * FROM addresses WHERE address_id = $1 AND user_id = $2', [address_id, userId]);
            if (result.rows.length === 0) {
...
app.patch('/api/user/addresses/:address_id', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            ...
            const ownershipResult = await client.query('SELECT type FROM addresses WHERE address_id = $1 AND user_id = $2', [address_id, userId]);
            ...
            if (validatedData.is_default) {
                ...
                await client.query('UPDATE addresses SET is_default = false WHERE user_id = $1 AND type = $2 AND address_id != $3', [userId, addressType, address_id]);
            }
...
app.delete('/api/user/addresses/:address_id', authenticateToken, async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query('DELETE FROM addresses WHERE address_id = $1 AND user_id = $2 RETURNING address_id', [address_id, userId]);
            if (result.rows.length === 0) {
                return res.status(404).json(createErrorResponse('Address not found', null, 'ADDRESS_NOT_FOUND'));
            }
            res.status(204).send();
...
app.patch('/api/user/addresses/:address_id/set-default', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            ...
            const addressResult = await client.query('SELECT type FROM addresses WHERE address_id = $1 AND user_id = $2', [address_id, userId]);
            if (addressResult.rows.length === 0) {
                return res.status(404).json(createErrorResponse('Address not found', null, 'ADDRESS_NOT_FOUND'));
            }
            const addressType = addressResult.rows[0].type;
            // Unset current default for this type
            await client.query('UPDATE addresses SET is_default = false WHERE user_id = $1 AND type = $2', [userId, addressType]);
            // Set new default
            const result = await client.query('UPDATE addresses SET is_default = true, updated_at = $1 WHERE address_id = $2 AND user_id = $3 RETURNING *', [new Date().toISOString(), address_id, userId]);
            await client.query('COMMIT');
            res.json(result.rows[0]);
        }
        catch (error) {
...
app.get('/api/wishlist', authenticateToken, async (req, res) => {
    try {
      ...
      `;
            ...
            // Get total count
            const countResult = await client.query('SELECT COUNT(*) as total FROM wishlist_items wi JOIN products p ON wi.product_id = p.product_id WHERE wi.user_id = $1 AND p.is_active = true', [userId]);
            const total = parseInt(countResult.rows[0].total);
            const items = result.rows.map(item => ({
...
app.post('/api/wishlist', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            ...
            const productResult = await client.query('SELECT product_id FROM products WHERE product_id = $1 AND is_active = true', [validatedData.product_id]);
            if (productResult.rows.length === 0) {
                return res.status(404).json(createErrorResponse('Product not found or inactive', null, 'PRODUCT_NOT_FOUND'));
            }
            // Check if already in wishlist
            const existingResult = await client.query('SELECT wishlist_item_id FROM wishlist_items WHERE user_id = $1 AND product_id = $2', [validatedData.user_id, validatedData.product_id]);
...
app.delete('/api/wishlist/:wishlist_item_id', authenticateToken, async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query('DELETE FROM wishlist_items WHERE wishlist_item_id = $1 AND user_id = $2 RETURNING wishlist_item_id', [wishlist_item_id, userId]);
            if (result.rows.length === 0) {
...
app.delete('/api/wishlist/product/:product_id', authenticateToken, async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query('DELETE FROM wishlist_items WHERE product_id = $1 AND user_id = $2 RETURNING wishlist_item_id', [product_id, userId]);
            if (result.rows.length === 0) {
...
app.get('/api/products/:product_id/reviews', async (req, res) => {
    try {
        ...
        const isVerifiedPurchase = is_verified_purchase;
        const limitNum = parseInt(limit) || 20;
        const offsetNum = parseInt(offset) || 0;
        const client = await pool.connect();
        try {
            // Verify product exists
            const productResult = await client.query('SELECT product_id FROM products WHERE product_id = $1 AND is_active = true', [product_id]);
            if (productResult.rows.length === 0) {
                return res.status(404).json(createErrorResponse('Product not found', null, 'PRODUCT_NOT_FOUND'));
            }
            let query = `
...
app.post('/api/products/:product_id/reviews', authenticateToken, async (req, res) => {
    try {
        ...
        `, [validatedData.user_id, product_id]);
            ...
            const existingReviewResult = await client.query('SELECT review_id FROM product_reviews WHERE user_id = $1 AND product_id = $2', [validatedData.user_id, product_id]);
...
app.post('/api/promo-codes/validate', async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query('SELECT * FROM promo_codes WHERE code = $1 AND is_active = true', [code.toUpperCase()]);
            if (result.rows.length === 0) {
                return res.json({
                    valid: false,
...
app.post('/api/newsletter/subscribe', async (req, res) => {
    try {
        ...
        try {
            ...
            const existingResult = await client.query('SELECT subscription_id, is_active FROM newsletter_subscriptions WHERE email = $1', [validatedData.email.toLowerCase()]);
            if (existingResult.rows.length > 0) {
                ...
                if (existing.is_active) {
                    ...
                    const result = await client.query('UPDATE newsletter_subscriptions SET is_active = true, subscribed_at = $1, unsubscribed_at = NULL WHERE subscription_id = $2 RETURNING *', [new Date().toISOString(), existing.subscription_id]);
...
app.post('/api/newsletter/unsubscribe', async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query('UPDATE newsletter_subscriptions SET is_active = false, unsubscribed_at = $1 WHERE email = $2 AND is_active = true RETURNING subscription_id', [new Date().toISOString(), email.toLowerCase()]);
            if (result.rows.length === 0) {
...
app.post('/api/admin/auth/login', async (req, res) => {
    try {
        ...
        try {
            // Find admin user
            const result = await client.query('SELECT * FROM admin_users WHERE username = $1 AND is_active = true', [username]);
            if (result.rows.length === 0) {
            ...
            const token = jwt.sign({ admin_id: admin.admin_id, username: admin.username, role: admin.role }, JWT_SECRET, { expiresIn: '8h' });
...
app.post('/api/admin/auth/logout', authenticateAdmin, async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            await client.query('UPDATE admin_sessions SET is_active = false WHERE token_hash = $1', [token]);
...
app.get('/api/admin/products', authenticateAdmin, async (req, res) => {
    try {
        const { search_query, is_active, is_featured, category_id, limit = 20, offset = 0 } = req.query;
        // Type cast query parameters
        const searchQuery = search_query;
...
app.patch('/api/admin/products/:product_id/images/:image_id', authenticateAdmin, async (req, res) => {
    try {
        ...
        try {
            ...
            const imageResult = await client.query('SELECT image_id FROM product_images WHERE image_id = $1 AND product_id = $2', [image_id, product_id]);
            if (imageResult.rows.length === 0) {
                return res.status(404).json(createErrorResponse('Product image not found', null, 'IMAGE_NOT_FOUND'));
            }
            // Handle primary image logic
            if (validatedData.is_primary) {
                await client.query('UPDATE product_images SET is_primary = false WHERE product_id = $1 AND image_id != $2', [product_id, image_id]);
            }
            // Build update query dynamically
            const updates = [];
            const values = [];
...
app.delete('/api/admin/products/:product_id/images/:image_id', authenticateAdmin, async (req, res) => {
    try {
        ...
        try {
            ...
            const imageResult = await client.query('SELECT is_primary FROM product_images WHERE image_id = $1 AND product_id = $2', [image_id, product_id]);
            if (imageResult.rows.length === 0) {
                return res.status(404).json(createErrorResponse('Product image not found', null, 'IMAGE_NOT_FOUND'));
            }
            ...
            if (wasPrimary) {
                await client.query('UPDATE product_images SET is_primary = true WHERE product_id = $1 ORDER BY display_order LIMIT 1', [product_id]);
            }
            await client.query('COMMIT');
            res.status(204).send();
        }
...
app.get('/api/admin/orders', authenticateAdmin, async (req, res) => {
    try {
        const { status, payment_status, search_query, date_from, date_to, limit = 20, offset = 0 } = req.query;
        // Type cast query parameters
        const statusFilter = status;
        const paymentStatusFilter = payment_status;
        const searchQuery = search_query;
...
app.patch('/api/admin/orders/:order_id', authenticateAdmin, async (req, res) => {
    try {
        ...
        try {
            ...
            const currentOrderResult = await client.query('SELECT status, payment_status FROM orders WHERE order_id = $1', [order_id]);
...
app.get('/api/admin/customers', authenticateAdmin, async (req, res) => {
    try {
        const { search_query, is_active, email_verified, limit = 20, offset = 0 } = req.query;
        // Type cast query parameters
        const searchQuery = search_query;
        const isActive = is_active;
...
app.get('/api/admin/customers/:user_id', authenticateAdmin, async (req, res) => {
    try {
        ...
        try {
            ...
            const customerResult = await client.query('SELECT user_id, email, first_name, last_name, phone, email_verified, is_active, created_at, updated_at FROM users WHERE user_id = $1', [user_id]);
...
app.post('/api/admin/inventory/:product_id/adjust', authenticateAdmin, async (req, res) => {
    try {
        ...
        try {
            ...
            const productResult = await client.query('SELECT stock_quantity FROM products WHERE product_id = $1 AND is_active = true', [product_id]);
            if (productResult.rows.length === 0) {
                return res.status(404).json(createErrorResponse('Product not found or inactive', null, 'PRODUCT_NOT_FOUND'));
            }
            ...
            await client.query('UPDATE products SET stock_quantity = $1, updated_at = $2 WHERE product_id = $3', [validatedData.new_quantity, new Date().toISOString(), product_id]);
    ...
    try {
        ...
        try {
            ...
            if (validatedData.name) {
                const nameCheckResult = await client.query('SELECT category_id FROM categories WHERE name = $1 AND category_id != $2', [validatedData.name, category_id]);
